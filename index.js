// index.js
// Minimal WhatsApp Bot: QR login + auto-view status + react + .status command
...
app.get('/qr', async (req, res) => {
  if (!latestQR)
    return res.send('<h2>Waiting for QR...</h2><p>Keep this page open — auto-refreshing.</p><script>setTimeout(() => location.reload(), 2500)</script>');
...
async function startBot() {
  try {
    ...
    // Auto-view + react + owner commands
    sock.ev.on('messages.upsert', async (m) => {
      try {
        if (!m.messages) return;
        const msg = m.messages[0];
        const owner = "254748397839@s.whatsapp.net"; // your number
        ...
        if (msg.key.remoteJid === owner && text === '.status') {
    ...
    // Extra status checks using message-info.update
    sock.ev.on('message-info.update', async (updates) => {
      ...
      for (const u of list) {
        if (u.key?.remoteJid === 'status@broadcast') {
          ...
          const emoji = EMOJIS[Math.floor(Math.random() * EMOJIS.length)];
          try {
            await sock.sendMessage(u.key.participant || u.key.remoteJid, {
              react: { text: emoji, key: u.key }
            });
          } catch {}
        }
      }
